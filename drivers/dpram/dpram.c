		...
		const unsigned char *buf, int len)
{
	...
	if (head < tail) { /* +++++++++ head ---------- tail ++++++++++ */
		size = tail - head - 1;
		if (len > size) {
			printk(KERN_INFO "BUFFER OVERFLOW SO RETURN FAIL len = %d, size =%d\n"
					, len, size);
			up(&write_mutex);
			return -EAGAIN;
		}
...
static void dpram_extra_mem_rw(struct _param_em *param)
{
	/* @LDK@ write */
	if (param->rw) {
		/* we don't have KBL so let's protect it ourself. */
				...
				unsigned long arg)
{
	...
	case HN_DPRAM_MEM_RW:
		{
			...
			val = copy_from_user((void *)&param, (void *)arg,
					sizeof(param));
			dpram_mem_rw(&param);
	...
	case HN_DPRAM_EXTRA_MEM_RW:
		{
			struct _param_em param;
			dprintk("HN_DPRAM_EXTRA_MEM_RW\n");
			val = copy_from_user((void *)&param, (void *)arg,
					sizeof(param));
			dpram_extra_mem_rw(&param);
...
static void param_init()
{
	...
	/*Temporary change to ensure that the device in debug level high*/
...
/* trebon_dpram_port_temp test with smem_alloc2 before modem is setup */;
	...
	do {
		SmemBase = (unsigned char *)
			(smem_alloc2(SMEM_ID_VENDOR0, 0x4000*2));
	} while(0);
#else
	SmemBase = (unsigned char *)
		(smem_alloc(SMEM_ID_VENDOR0, 0x4000*2));
#endif
	if (!SmemBase) {
