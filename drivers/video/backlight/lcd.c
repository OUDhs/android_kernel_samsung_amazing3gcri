...
static unsigned int lcd_Idtype;

static struct device *sec_lcd_dev;
...
static DEVICE_ATTR(lcd_type, 0644, lcdtype_show, NULL);
#endif
...
static int __init lcd_class_init(void)
{
	...
	#if defined(CONFIG_MACH_AMAZING_CDMA)
	 sec_lcd_dev = device_create(lcd_class, NULL, 0, NULL, "panel");
if (IS_ERR(sec_lcd_dev))
	pr_err("Failed to create device(ts)!\n");
if (device_create_file(sec_lcd_dev, &dev_attr_lcd_type) < 0)
