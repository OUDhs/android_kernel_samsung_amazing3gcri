...
#define TOTAL_RECHARGING_TIME			(2 * 60*60*HZ)
#else
...
#if defined(CONFIG_MACH_AMAZING3G)
#define BATT_CHECK_INTERVAL                     (10 * HZ)
#define START_RCOMP                             69
#define SOC_ALR_THR                             0x1E
#if defined(CONFIG_MACH_AMAZING3G_CRI)
#define TEMP_COHOT                              -4
#define TEMP_COCOLD                             -59
#else
#define TEMP_COHOT                              -1
#define TEMP_COCOLD                             -45
#endif
#else
#define BATT_CHECK_INTERVAL                     (5 * HZ)
#endif
#define BATT_FULL_SHOW_INTERVAL			(30 * HZ)
#define AVERAGE_COUNT_CHG_CURRENT	5
#else
...
#elif CONFIG_MACH_AMAZING_CDMA
#ifdef CONFIG_MACH_AMAZING3G
#define BATT_TEMP_EVENT_BLOCK                   240
#ifdef CONFIG_MACH_AMAZING3G_CRI
#define BATT_TEMP_HIGH_BLOCK                    330
#else
#define BATT_TEMP_HIGH_BLOCK			320
#endif
#define BATT_TEMP_HIGH_RECOVER			410
#define BATT_TEMP_LOW_BLOCK			796
#define BATT_TEMP_LOW_RECOVER			764

#define BATT_TEMP_HIGH_BLOCK_LPM		385
#define BATT_TEMP_HIGH_RECOVER_LPM		413
#define BATT_TEMP_LOW_BLOCK_LPM			796
#define BATT_TEMP_LOW_RECOVER_LPM		766
#else
#define BATT_TEMP_EVENT_BLOCK			276
#define BATT_TEMP_HIGH_BLOCK			240
#define BATT_TEMP_HIGH_RECOVER			410
#define BATT_TEMP_LOW_BLOCK			805
#define BATT_TEMP_LOW_RECOVER			766

#define BATT_TEMP_HIGH_BLOCK_LPM		385
#define BATT_TEMP_HIGH_RECOVER_LPM		413
#define BATT_TEMP_LOW_BLOCK_LPM			781
#define BATT_TEMP_LOW_RECOVER_LPM		766
#endif
...
#elif defined(CONFIG_MACH_AMAZING_CDMA)
#if defined(CONFIG_MACH_AMAZING3G)
/*charging terminates at 100 mA*/
#define BATT_FULL_CHARGING_CURRENT	130
/*charginh UI displays FULL at 150 mA*/
#define BATT_FULL_CHARGING_CURRENT1	180
#else
#define BATT_FULL_CHARGING_CURRENT	140
#endif
#define BATT_FULL_CHARGING_VOLTAGE      4000
#define BATT_FULL_CHARGING_VOLTAGE_CP   4000
#else
#define BATT_FULL_CHARGING_CURRENT	90
#define BATT_FULL_CHARGING_VOLTAGE      4180
...
#elif defined(CONFIG_MACH_AMAZING_CDMA)
#define BATT_RECHARGING_VOLTAGE_1	4125
...
#define BATT_TEMP_ADC_TOLERANCE 25
#else
#define BATT_TEMP_ADC_TOLERANCE 0
#endif
...
#if defined(CONFIG_MACH_AMAZING3G_CRI)
	MSM_BATTERY_ATTR(slate_test_mode),
#endif
...
void full_batt_show_timer_func(unsigned long param)
{
	struct msm_battery_info *chg = (struct msm_battery_info *)param;
	msm_batt_info.set_full_batt_show = 0;
	queue_work(msm_batt_info.msm_batt_wq, &msm_batt_work);
}
#endif
				       ...
				       char *buf)
{
	...
	case SLATE_TEST_MODE:
		i += scnprintf(buf + i, PAGE_SIZE - i, "%d\n",
			msm_batt_info.slate_test_mode_variable);
		break;
#endif
...
#else
		temporary_adc = msm_batt_info.battery_temp_adc;
		i += scnprintf(buf + i, PAGE_SIZE - i, "%d\n",
					(1000-temporary_adc));
#endif
		break;
				       ...
				       const char *buf, size_t count)
{
	...
	case SLATE_TEST_MODE:
		if (sscanf(buf, "%d\n", &x) == 1) {
			printk("[usb_amazing] %s:  %d\n" , __func__, x);
			msm_batt_info.slate_test_mode_variable = x;
		if (msm_batt_info.charging_source ==
				USB_CHG && !(charging_boot)) {
			if (!msm_batt_info.slate_test_mode_variable) {
				if (msm_batt_info.batt_status ==
					POWER_SUPPLY_STATUS_CHARGING) {
					if (!((msm_batt_info.batt_full_check) ||
					(msm_batt_info.batt_full_display))) {
						msm_batt_info.batt_status =
					POWER_SUPPLY_STATUS_NOT_CHARGING;
						msm_batt_chg_en(STOP_CHARGING);
						pr_info("%s:SLATE_TEST Stop Charging-status(%d)\n", __func__, msm_batt_info.batt_status);
					}
				}
			} else{
			if ((!msm_batt_info.ovp_status) &&
				(!(msm_batt_info.batt_vf_adc > 500)) &&
				...
				if (!((msm_batt_info.batt_full_check)
					...
					pr_info("%s :SLATE_TEST	Start Charging-status(%d)\n", __func__, msm_batt_info.batt_status);
				}
			}
			}
			}
...
#if defined(CONFIG_MACH_AMAZING3G)
		msm_batt_info.batt_full_display = 0;
#endif
		msm_batt_chg_en(false);
		wake_unlock(&vbus_wake_lock);
		#ifdef CONFIG_MACH_AMAZING_CDMA
			if (chg->set_full_batt_show)
				mod_timer(&msm_batt_info.full_batt_show_timer,
					jiffies + BATT_FULL_SHOW_INTERVAL);
		#endif
	} else {
		#ifdef CONFIG_MACH_AMAZING_CDMA

		if (chg->set_full_batt_show)
			chg->set_full_batt_show = 0;
		#endif
...
{
	union power_supply_propval value;
	static unsigned int time_after_vol4;
...
#elif defined(CONFIG_MACH_AMAZING_CDMA)
				} else if ((msm_batt_info.battery_vol < 4125)
						&& (msm_batt_info.batt_full_check == 0)
						&& (value.intval == 100)) {
					if (time_after_vol4 == 0) {
						time_after_vol4 = jiffies;
					} else {
						if (time_after((unsigned long)jiffies,
							(unsigned long)(time_after_vol4
								+ TOTAL_WATING_TIME))) {
							value.intval = 99;
							time_after_vol4 = 0;
						}
...
#ifdef CONFIG_MACH_AMAZING_CDMA
			if (msm_batt_info.set_full_batt_show)
				value.intval = 100;
#endif
...
static void msm_batt_chg_en(chg_enable_type enable)
{
	...
	if ((prev_charging == enable) &&
		(prev_cable == msm_batt_info.cable_status) &&
		(prev_acc == msm_batt_info.acc_status))
		return 0;

	prev_charging = enable;
	prev_cable = msm_batt_info.cable_status;
	prev_acc = msm_batt_info.acc_status;
	mutex_lock(&msm_batt_info.mutex);
#endif

/*Added to check Slate status when charging controls changed*/
#if defined(CONFIG_MACH_AMAZING3G_CRI)
	if (msm_batt_info.charging_source ==
			USB_CHG && !(charging_boot)) {
		if ((!msm_batt_info.slate_test_mode_variable) &&
				(enable == START_CHARGING)) {
				/* Ignore slate state during recharging phases*/
			if (!((msm_batt_info.batt_full_check) ||
					(msm_batt_info.batt_full_display))) {
				msm_batt_info.batt_status =
					POWER_SUPPLY_STATUS_NOT_CHARGING;
				pr_info("%s : [BATT] SLATE_TEST batt_status(%d) disable charging\n", __func__, msm_batt_info.batt_status);
...
#if defined(CONFIG_MACH_AMAZING3G_CRI)
			msm_batt_info.fg_rcomp = 120;
			value.intval = (msm_batt_info.fg_rcomp << 8)
					+ SOC_ALR_THR;

			pr_info("[BATT] %s: value.intval : %x\n",
...
static int msm_batt_average_chg_current(int chg_current_adc)
{
	static int history[AVERAGE_COUNT_CHG_CURRENT] = {0};
	static int count;
	static int index;
	...
	if (chg_current_adc < 0) {
		count = 0;
		index = 0;
		for (i = 0; i < AVERAGE_COUNT_CHG_CURRENT; i++)
			history[i] = 0;
	...
	ret = ((sum-max-min) / (AVERAGE_COUNT_CHG_CURRENT-2));

	index++;

	if (index == AVERAGE_COUNT_CHG_CURRENT) {
		history[0] = ret;
		index = 1;
	}
	...
	if (count < AVERAGE_COUNT_CHG_CURRENT)
...
static int msm_batt_check_full_charging(int chg_current_adc)
{
	...
	static unsigned int time_after_under_tsh1;
	...
	if (msm_batt_is_over_abs_time()) {
		...
		msm_batt_info.batt_status = POWER_SUPPLY_STATUS_FULL;
		#ifdef CONFIG_MACH_AMAZING_CDMA
		msm_batt_info.set_full_batt_show = 1;
		#endif
		msm_batt_info.batt_capacity = 100;
		msm_batt_chg_en(STOP_CHARGING);
		return 1;
...
#if defined(CONFIG_MACH_AMAZING3G)
	pr_info("[BATT] %s: (voltage=%d, PMIC voltage= %d, ICHG=%d, capacity=%d)\n",
			__func__,
			msm_batt_info.battery_vol,
...
#ifndef CONFIG_CHARGER_SMB328A
#if defined(CONFIG_MACH_AMAZING_CDMA)
	if ((msm_batt_info.battery_vol >= BATT_FULL_CHARGING_VOLTAGE) || (rep_batt_chg.v1.battery_voltage >= BATT_FULL_CHARGING_VOLTAGE_CP)) {
#else
		...
		if (chg_current_adc < BATT_FULL_CHARGING_CURRENT &&
			...
			if (time_after_under_tsh == 0) {
				...
				if (time_after((unsigned long)jiffies,
					...
					pr_info("[BATT] %s: Fully charged: Charging Stopped, "
						"(voltage=%d, ICHG=%d)\n",
						__func__,
						msm_batt_info.battery_vol,
						chg_current_adc);
...
if (msm_batt_info.batt_recharging != 1) {
	if ((msm_batt_info.battery_vol >= BATT_FULL_CHARGING_VOLTAGE) || (rep_batt_chg.v1.battery_voltage >= BATT_FULL_CHARGING_VOLTAGE_CP)) {
		if (chg_current_adc < BATT_FULL_CHARGING_CURRENT1 &&
			msm_batt_info.batt_capacity >= 99)	{
			if (time_after_under_tsh1 == 0) {
				time_after_under_tsh1 = jiffies;
			} else {
				if (time_after((unsigned long)jiffies,
					(unsigned long)(time_after_under_tsh1
					+ TOTAL_WATING_TIME))) {
					pr_info("[BATT] %s: Fully charged : Full Display, "
						"(voltage=%d, ICHG=%d)\n",
						__func__,
						msm_batt_info.battery_vol,
						chg_current_adc);
					msm_batt_info.batt_full_check = 1;
					msm_batt_info.batt_full_display = 1;
					msm_batt_info.batt_recharging = 0;
					msm_batt_info.batt_status =
						POWER_SUPPLY_STATUS_FULL;
					msm_batt_info.batt_capacity = 100;
					time_after_under_tsh1 = 0;
					return 1;
				}
			}
		} else {
...
static int msm_batt_check_recharging(void)
{
	...
	if (msm_batt_info.battery_vol <= BATT_RECHARGING_VOLTAGE_1) {
		...
		if (time_after((unsigned long)jiffies,
			...
			msm_batt_info.batt_recharging = 1;
			msm_batt_chg_en(START_CHARGING);
#if !defined(CONFIG_MACH_KYLE) && !defined(CONFIG_MACH_AMAZING_CDMA)
			msm_batt_info.batt_status =
					POWER_SUPPLY_STATUS_CHARGING;
#endif
			return 1;
...
{
	static unsigned int time_after_vol3;
...
&& (msm_batt_info.batt_full_check == 0) && (battery_level == 100)) {
	...
	if (pmic_gpio_get_value(PMIC_GPIO_1) &&
		...
		battery_level = 1;
			...
			&& (battery_level == 100))
	{
		if (time_after_vol3 == 0) {
			time_after_vol3 = jiffies;
		} else{
			if (time_after((unsigned long)jiffies,
						(unsigned long)(time_after_vol3
							+ TOTAL_WATING_TIME))) {
				battery_level = 99;
				time_after_vol3 = 0;
			}
...
static int msm_batt_control_temperature(int temp_adc)
{
	...
	static char *health_text[] = {
		...
		if (temp_adc <= BATT_TEMP_ADC_TOLERANCE)
			return 0;
#endif
...
static void msm_batt_update_psy_status(void)
{
	...
	/*(+)Open Check*/
	if (!flag) {
		/* To avoid blocking current during boot*/
		/*charging current blocks due to RPC call failure*/
#ifndef CONFIG_MACH_AMAZING_CDMA
		msm_batt_chg_en(STOP_CHARGING);
		pr_info("[BATT] %s: STOP Charging\n", __func__);
		msleep(3);
	...
	if (1 == charging_boot) {
		if (msm_batt_info.batt_status ==
				POWER_SUPPLY_STATUS_CHARGING) {
			battery_temp_adc = battery_temp_adc +
					 BATT_TEMP_ADC_TOLERANCE;
		}
	}
#endif
#if defined(CONFIG_MACH_AMAZING_CDMA) || defined(CONFIG_MACH_KYLE)
...
#if defined(CONFIG_MACH_AMAZING3G)
#if defined(CONFIG_MAX17043_FUELGAUGE)
	if ((msm_batt_info.battery_temp / 10) > 20) {
		new_rcomp = msm_batt_info.fg_rcomp +
		(((msm_batt_info.battery_temp / 10) - 20) *
		(TEMP_COHOT) / 10);
		pr_info("[BATT] %s: new_rcomp : %x\n", __func__, new_rcomp);
	} else if ((msm_batt_info.battery_temp / 10) < 20) {
		new_rcomp = msm_batt_info.fg_rcomp +
		(((msm_batt_info.battery_temp / 10) - 20) *
		(TEMP_COCOLD) / 10);
		pr_info("[BATT] %s: at low temp new_rcomp : %x\n",
		__func__, new_rcomp);
	} else {
		new_rcomp = msm_batt_info.fg_rcomp;
	}
	if (new_rcomp > 255)
		...
		new_rcomp = 0;
	value.intval = (new_rcomp << 8) + SOC_ALR_THR;
	pr_info("[BATT] %s: value.intval : %x\n",
		__func__, value.intval);
	msm_batt_info.pdata->psy_fuelgauge->set_property(
		msm_batt_info.pdata->psy_fuelgauge,
...
#if defined(CONFIG_MACH_AMAZING3G_CRI)
	msm_batt_info.slate_test_mode_variable = 1;
#endif
...
#ifdef CONFIG_MACH_AMAZING_CDMA
	setup_timer(&msm_batt_info.full_batt_show_timer,
			full_batt_show_timer_func, 0);
	msm_batt_info.set_full_batt_show = 0;
#endif
...
#ifdef CONFIG_MACH_AMAZING_CDMA
	mutex_destroy(&msm_batt_info.mutex);
#endif
