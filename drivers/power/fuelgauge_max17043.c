...
static int max17043_reset_chip(struct i2c_client *client)
{
	int ret;
	u16 rst_cmd = 0x4000;

	ret = i2c_smbus_write_word_data(client,
			MAX17043_MODE_MSB, swab16(rst_cmd));
	msleep(500);
                            ...
                            union power_supply_propval *val)
{
	...
	u16 temp;
		...
		if (chip->rcomp != val->intval) {
			i2c_smbus_write_word_data(chip->client,
				MAX17043_RCONFIG_MSB, swab16(val->intval));

			chip->rcomp = val->intval;
			temp = max17043_read_reg(chip->client,
				MAX17043_RCONFIG_MSB);
			printk(KERN_DEBUG "%s: rconfig = %x\n",
				 __func__, swab16(temp));
		}
                break;
        case POWER_SUPPLY_PROP_FUELGAUGE_RESET :
			max17043_reset_chip(chip->client);
...
static void max17043_get_vcell(struct i2c_client *client)
{
	...
	u8 lsb;
	u16 temp;
	temp = max17043_read_reg(client, MAX17043_VCELL_MSB);
	msb  = (temp & 0xff) & 0xff;
	lsb = (temp >> 8) & 0xff;
...
static void max17043_get_soc(struct i2c_client *client)
{
	...
	u32 CalcStep2;
	int socValue;
	temp = max17043_read_reg(client, MAX17043_SOC_MSB);
	msb = (temp & 0xff) & 0xff;
	lsb = (temp >> 8) & 0xff;
	...
	if (socValue >= 0)
		adj_soc = (((socValue*10)/10)*100000)/1020;
	else
		adj_soc = 0;
#else
	if (socValue >= 0)
		adj_soc = ((((socValue*10)-0)/10)*100000)/(970);
	else
		adj_soc = 0;
#endif
...
#else
	if (socValue >= 0)
		adj_soc = ((((socValue*10)-0)/10)*100000)/(985);
...
static void max17043_get_version(struct i2c_client *client)
{
	...
	u8 lsb;
	u16 temp;
	temp = max17043_read_reg(client, MAX17043_VER_MSB);
			...
			const struct i2c_device_id *id)
{
	...
	temp = max17043_read_reg(chip->client, MAX17043_RCONFIG_MSB);

	chip->rcomp = swab16(temp);
	printk(KERN_DEBUG "%s: rconfig = %x\n",
		__func__, chip->rcomp);
