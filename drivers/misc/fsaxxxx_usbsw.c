...
static int usb_state_flag;
#ifdef CONFIG_MACH_AMAZING_CDMA
static int intr2_reg;
#endif
...
/* WORKAROUND for not recognizing ta detaching */
	...
	if (intr & (1 << 0)) {
		...
		if (val1 & FSA_DEV_T1_CHARGER_MASK) {
			...
			mdelay(250);
#endif
			fsausb_read_reg(client, FSA9480_REG_VBUS, &vbus);
		...
		if (val1 == 0 && val2 == 0) {
			...
			fsausb_write_reg(client, FSA_REG_CTRL, 0x1E);
#ifdef CONFIG_MACH_AMAZING_CDMA
		} else if ((intr & (1 << 5)) && (intr & (1 << 7))) {
			pr_info("[BATT] OVP and OVP Recover condition occuring simultaneosly in deep sleep..!!\n");
#endif
		} else if (intr & (1 << 5)) {
			if (pdata->ovp_cb)
