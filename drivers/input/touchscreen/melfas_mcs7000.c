...
static ssize_t tsp_recommend_firmware_show(struct device *dev,\
		struct device_attribute *attr, char *buf);
static void set_cmd_result(char *buff, int len);
static void not_support_cmd(struct device *dev, struct device_attribute *attr,
						char *buf);
static void run_cm_abs_read(struct device *dev,
	struct device_attribute *attr, char *buf);
static void run_cm_delta_read(struct device *dev,
	struct device_attribute *attr, char *buf);
...
struct tsp_cmd tsp_cmds[] = {
	...
	{TSP_CMD("get_fw_ver_bin", tsp_recommend_firmware_show),},
...
static void get_raw_data_all(/*struct melfas_ts_driver *info,*/ u8 cmd)
{
	u8 w_buf[6];
	u8 read_buffer[2];	/* 52 */
	int RX_NUM = 11, TX_NUM = 15;
	int ret;
	int i, j;
	...
	if (cmd == MMS_VSC_CMD_EXIT) {
		...
		ret = i2c_smbus_write_i2c_block_data(melfas_ts->client,
						     w_buf[0], 5, &w_buf[1]);
		if (ret < 0)
			goto err_i2c;
		enable_irq(melfas_ts->client->irq);
	...
	for (i = 0; i <  RX_NUM; i++) {
		for (j = 0; j < TX_NUM; j++) {
			...
			ret = i2c_smbus_read_i2c_block_data(melfas_ts->client,
				0xBF, 2, read_buffer);
			if (ret < 0)
				goto err_i2c;
			...
			if (cmd == MMS_VSC_CMD_INTENSITY) {
				melfas_ts->intensity[j * RX_NUM + i] = raw_data;
				dev_dbg(&melfas_ts->client->dev, "[TSP] intensity[%d][%d] = %d\n",
				i, j, melfas_ts->intensity[j * RX_NUM + i]);
			} else if (cmd == MMS_VSC_CMD_CM_DELTA) {
				melfas_ts->inspection[j * RX_NUM + i]
					= raw_data;
				dev_dbg(&melfas_ts->client->dev, "[TSP] delta[%d][%d] = %d\n",
				i, j, melfas_ts->inspection[j * RX_NUM + i]);
			} else if (cmd == MMS_VSC_CMD_CM_ABS) {
				melfas_ts->raw[j * RX_NUM + i] = raw_data;
				dev_dbg(&melfas_ts->client->dev, "[TSP] raw[%d][%d] = %d\n",
					i, j, melfas_ts->raw[j * RX_NUM + i]);
			} else if (cmd == MMS_VSC_CMD_REFER) {
				melfas_ts->reference[j * RX_NUM + i] =
						raw_data >> 3;
				dev_dbg(&melfas_ts->client->dev, "[TSP] reference[%d][%d] = %d\n",
				i, j, melfas_ts->reference[j * RX_NUM + i]);
			}
		}
	...
	set_cmd_result(buff, strnlen(buff, sizeof(buff)));

	enable_irq(melfas_ts->client->irq);

err_i2c:
	dev_err(&melfas_ts->client->dev, "%s: fail to i2c (cmd=%d)\n",
		__func__, cmd);
}
	...
	(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	return snprintf(buf, 3, "%d", menu_key_sensitivity);
}
	...
	(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	return snprintf(buf, 3, "%d", home_key_sensitivity);
}
	...
	(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	return snprintf(buf, 3, "%d", back_key_sensitivity);
}
...
static void touchkey_firm_version_phone_show(struct device *dev, \
		struct device_attribute *attr, char *buf)
{
	printk(KERN_DEBUG"[TSP] %s\n", __func__);
	snprintf(buf, 5, "0x%x\n", melfas_ts->fw_ver);
		...
		struct device_attribute *attr, char *buf)
{
	printk(KERN_DEBUG"[TSP] %s\n", __func__);
	set_default_result();
	get_raw_data_all(MMS_VSC_CMD_CM_ABS);
	get_raw_data_all(MMS_VSC_CMD_EXIT);
	melfas_ts->cmd_state = 2;
}
		...
		struct device_attribute *attr, char *buf)
{
	printk(KERN_DEBUG"[TSP] %s\n", __func__);
	set_default_result();
	get_raw_data_all(MMS_VSC_CMD_CM_DELTA);
	get_raw_data_all(MMS_VSC_CMD_EXIT);
	melfas_ts->cmd_state = 2;
}
...
static ssize_t config_tsp_version_show(struct device *dev,
				 struct device_attribute *attr, char *buf)
{
	/* FW version 0x07 released on 3rd July,
	FW version 0x11 released on 14th August*/
	if (melfas_ts->fw_ver >= 0x07)
	#if defined(CONFIG_MACH_AMAZING3G_CRI) \
		&& defined(CONFIG_MACH_AMAZING_CDMA)
		return sprintf(buf, "R740C_ME_0926\n");
	#elif defined(CONFIG_MACH_AMAZING3G) \
		&& defined(CONFIG_MACH_AMAZING_CDMA)
		return sprintf(buf, "S738C_ME_0926\n");
	#else
		return sprintf(buf, "S735C_ME_0926\n");
	#endif
	else
...
static DEVICE_ATTR(touchkey_firm_version_phone, S_IRUGO | S_IWUSR | S_IWGRP, \
		touchkey_firm_version_phone_show, NULL);
...
static u32 get_raw_data_two_1(u16 rx_idx, u16 tx_idx,
		u8 cmd)
{
	u8 w_buf[6];
	u8 read_buffer[2];
	...
	if (cmd != MMS_VSC_CMD_INTENSITY && cmd != MMS_VSC_CMD_RAW &&
		cmd != MMS_VSC_CMD_REFER && cmd != VSC_RAW_TK) {
		return FAIL;
	}
	...
	ret = i2c_smbus_write_i2c_block_data(melfas_ts->client, w_buf[0], 5,
			&w_buf[1]);
	if (ret < 0)
		goto err_i2c;

	ret = i2c_smbus_read_i2c_block_data(melfas_ts->client, 0xBF, 2,
			read_buffer);
	if (ret < 0)
		goto err_i2c;
				...
				struct device_attribute *attr, char *buf)
{
	...
	tkey_threshold = i2c_smbus_read_byte_data(client,
						VSC_THRESHOLD_TK);
...
static ssize_t tkey_rawdata_show0(struct device *dev,
				struct device_attribute *attr, char *buf)
{
	struct i2c_client *client = melfas_ts->client;
	int ret;
	...
	ret = get_raw_data_two_1(0, 0, VSC_RAW_TK);
	if (ret < 0)
...
static ssize_t tkey_rawdata_show1(struct device *dev,
				struct device_attribute *attr, char *buf)
{
	struct i2c_client *client = melfas_ts->client;
	int ret;
	...
	ret = get_raw_data_two_1(0, 1, VSC_RAW_TK);
	if (ret < 0)
		dev_err(&client->dev, "%s: fail to read (%d)\n", __func__, ret);
...
static ssize_t tkey_rawdata_show2(struct device *dev,
				struct device_attribute *attr, char *buf)
{
	struct i2c_client *client = melfas_ts->client;
	int ret;
	...
	ret = get_raw_data_two_1(0, 2, VSC_RAW_TK);
	if (ret < 0)
		dev_err(&client->dev, "%s: fail to read (%d)\n", __func__, ret);
...
static DEVICE_ATTR(touchkey_raw_data0, S_IRUGO, tkey_rawdata_show0, NULL) ;
static DEVICE_ATTR(touchkey_raw_data1, S_IRUGO, tkey_rawdata_show1, NULL) ;
static DEVICE_ATTR(touchkey_raw_data2, S_IRUGO, tkey_rawdata_show2, NULL) ;
...
//		udelay(50);
	...
	if ((read_num > 0) && (read_num <= 66)) {
		...
		printk(KERN_DEBUG "[TSP] read_num: %d\n", read_num);
#endif
//		for (i = 0; i < 10; i++)
		for (i = 0; i < 5; i++)			
		{	
...
#ifdef __TOUCH_DEBUG__				
	printk(KERN_DEBUG "[TSP] keyID: %d, State: %d\n", keyID, touchState);
#endif				
				}
#endif	//  __TOUCH_KEY__	
				}			
...
#if defined (CONFIG_TOUCHSCREEN_MELFAS_TS)
	/* when firmware update fail, add com_type == 0x00,
	 condition to re-try update during booting...*/
	/*If TSP is modified, then Compatibility_group version must be changed.
	Therfore compatibilty_group version is not fixed so we are disabling
	the checking of compatiblity version from nowonwards as suggested by
	Vendor*/
	/*if((melfas_ts->com_type == Compatibility_group)
	|| (melfas_ts->com_type == 0x00))
	{*/
		if(recommend_ver > melfas_ts->fw_ver)
		{
...
/***********Added Support for *#86824# from PREVAIL2***********/
	if (device_create_file(sec_touchkey_dev,
