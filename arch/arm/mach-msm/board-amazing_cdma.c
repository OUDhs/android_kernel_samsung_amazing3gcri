...
#include <mach/gpio_amazing_cdma.h>
#else
...
#endif
#ifdef CONFIG_MAX17043_FUELGAUGE
...
#define ENABLE_WLAN_LDO
#endif

#define ENABLE_WLAN_LDO
#ifdef ENABLE_WLAN_LDO
...
{
	return current_key_state; /* refer from rpc_server_handset */
}
...
{
	return current_jack_type;	/* refer from rpc_server_handset */
}
...
static int sec_jack_read_adc(int channel, int *adc_data)
{
	...
	if (!ret) {
		pr_err("%s: wait interrupted channel %d ret=%d\n",
				__func__, channel, ret);
		//rohbt_ics		pm8058_xoadc_clear_recentQ();
		goto out;
	}
...
static int sec_jack_get_adc_value(void)
{
	...
	temp_adc = sec_jack_read_adc(5, &adc_data);
...
#ifdef CONFIG_PROXIMITY_SENSOR
int LED_onoff(int on)
{

	int rc = 0;
	printk("%s onoff:%d\n", __func__, on);
	rc = gpio_request(GPIO_PROX_LDO, "GPIO_PROX_LDO");
	if (rc < 0)
		pr_err("failed to request prox_ldo\n");
	rc = gpio_direction_output(GPIO_PROX_LDO, on);
	if (rc < 0)
		pr_err("failed to direction_output prox_ldo\n");
	gpio_free(GPIO_PROX_LDO);
	return 0;
}
static int gp2a_init()
{
	gpio_tlmm_config(GPIO_CFG(GPIO_PROX_LDO, 0, GPIO_CFG_OUTPUT,
			GPIO_CFG_PULL_UP, GPIO_CFG_2MA), GPIO_CFG_ENABLE);
	gpio_tlmm_config(GPIO_CFG(GPIO_PROXI_INT, 0, GPIO_CFG_INPUT,
			GPIO_CFG_PULL_UP, GPIO_CFG_2MA), GPIO_CFG_ENABLE);
	return 0;
}
...
static void __init register_i2c_devices(void)
{
	...
	printk("registration of devices");
#endif
...
static struct smb328a_platform_data smb328a_pdata = {
		...
		.chg_curr_ta = SMB328A_CURR_600,
		.chg_curr_ta_event = SMB328A_CURR_600,
        .chg_curr_usb = SMB328A_CURR_500,
		.chg_ac_limit_ta = SMB328A_LIMIT_600,
        .chg_ac_limit_usb = SMB328A_LIMIT_450,
        .chg_term_curr = SMB328A_TERM_200,
...
static void jena_deskdock_cb(u8 attached, struct fsausb_ops *ops)
{
	pr_info("[BATT] Board file [FSA880]: Charger Callback\n");
}
...
#define FUEL_I2C_SCL 123
#define FUEL_I2C_SDA 124
...
static struct i2c_board_info fg_smb_i2c_devices[] = {
    {
            I2C_BOARD_INFO("max17043", 0x6D>>1),
...
static int checkChgMode()
{
	return charging_boot;
}
#endif
...
#ifdef ENABLE_WLAN_LDO
int wlan_enable_ldo_33v(int on)
{

	int rc = 0, min_level = 2800000, max_level = 2800000;
	...
	wlan_reg = regulator_get(NULL, "wlan");

	if (IS_ERR_OR_NULL(wlan_reg)) {
			rc = PTR_ERR(wlan_reg);
			dev_err(NULL, "%s: could not get regulator %s: %d\n",
					__func__, "wlan", rc);
			goto reg_get_fail;
		}
	...
	rc = on ? regulator_set_voltage(wlan_reg,
				min_level,
					max_level) : 0;
	if (rc) {
		dev_err(NULL,
	...
	rc = on ? regulator_enable(wlan_reg) : 0;
	if (rc) {
		dev_err(NULL,
...
reg_get_fail:
printk(KERN_ERR  "%s %s --regulator get-fail\n", __func__, on ? "on" : "down");
regulator_put(wlan_reg);
wlan_reg = NULL;
return WLAN_ERROR;

reg_disable:
	printk(KERN_INFO "%s %s --reg_disable\n", __func__, on ? "on" : "down");
	if (!IS_ERR_OR_NULL(wlan_reg)) {
		regulator_disable(wlan_reg);
		regulator_put(wlan_reg);
		wlan_reg = NULL;
	}
	return rc;
...
static uint32_t lcdc_amazing_gpio_init_table[] = {
	GPIO_CFG(GPIO_LCD_MCLK, 0, GPIO_CFG_OUTPUT,
				GPIO_CFG_PULL_DOWN, GPIO_CFG_2MA),
	GPIO_CFG(GPIO_LCD_DETECT, 0, GPIO_CFG_INPUT,
				GPIO_CFG_NO_PULL, GPIO_CFG_2MA),
	GPIO_CFG(GPIO_LCD_VSYNC, 0, GPIO_CFG_OUTPUT,
				GPIO_CFG_NO_PULL, GPIO_CFG_2MA),
	GPIO_CFG(GPIO_LCD_HSYNC, 0, GPIO_CFG_OUTPUT,
				GPIO_CFG_NO_PULL, GPIO_CFG_2MA),

	GPIO_CFG(GPIO_LCD_R_0, 0, GPIO_CFG_OUTPUT,
			GPIO_CFG_PULL_DOWN, GPIO_CFG_2MA),
	GPIO_CFG(GPIO_LCD_R_1, 0, GPIO_CFG_OUTPUT,
			GPIO_CFG_PULL_DOWN, GPIO_CFG_2MA),
	GPIO_CFG(GPIO_LCD_R_2, 0, GPIO_CFG_OUTPUT,
			GPIO_CFG_PULL_DOWN, GPIO_CFG_2MA),
	GPIO_CFG(GPIO_LCD_R_3, 0, GPIO_CFG_OUTPUT,
			GPIO_CFG_PULL_DOWN, GPIO_CFG_2MA),
	GPIO_CFG(GPIO_LCDR_4, 0, GPIO_CFG_OUTPUT,
			GPIO_CFG_PULL_DOWN, GPIO_CFG_2MA),
	GPIO_CFG(GPIO_LCD_R_5, 0, GPIO_CFG_OUTPUT,
			GPIO_CFG_PULL_DOWN, GPIO_CFG_2MA),


	GPIO_CFG(GPIO_LCD_G_0, 0, GPIO_CFG_OUTPUT,
			GPIO_CFG_PULL_DOWN, GPIO_CFG_2MA),
	GPIO_CFG(GPIO_LCD_G_1, 0, GPIO_CFG_OUTPUT,
			GPIO_CFG_PULL_DOWN, GPIO_CFG_2MA),
	GPIO_CFG(GPIO_LCDE_G_2, 0, GPIO_CFG_OUTPUT,
			GPIO_CFG_PULL_DOWN, GPIO_CFG_2MA),
	GPIO_CFG(GPIO_LCD_G_3, 0, GPIO_CFG_OUTPUT,
			GPIO_CFG_PULL_DOWN, GPIO_CFG_2MA),
	GPIO_CFG(GPIO_LCD_G_4, 0, GPIO_CFG_OUTPUT,
			GPIO_CFG_PULL_DOWN, GPIO_CFG_2MA),
	GPIO_CFG(GPIO_LCD_G_5, 0, GPIO_CFG_OUTPUT,
			GPIO_CFG_PULL_DOWN, GPIO_CFG_2MA),

	GPIO_CFG(GPIO_LCD_B_0, 0, GPIO_CFG_OUTPUT,
			GPIO_CFG_PULL_DOWN, GPIO_CFG_2MA),
	GPIO_CFG(GPIO_LCD_B_1, 0, GPIO_CFG_OUTPUT,
			GPIO_CFG_PULL_DOWN, GPIO_CFG_2MA),
	GPIO_CFG(GPIO_LCD_B_2, 0, GPIO_CFG_OUTPUT,
			GPIO_CFG_PULL_DOWN, GPIO_CFG_2MA),
	GPIO_CFG(GPIO_LCD_B_3, 0, GPIO_CFG_OUTPUT,
			GPIO_CFG_PULL_DOWN, GPIO_CFG_2MA),
	GPIO_CFG(GPIO_LCD_B_4, 0, GPIO_CFG_OUTPUT,
			GPIO_CFG_PULL_DOWN, GPIO_CFG_2MA),
	GPIO_CFG(GPIO_LCD_B_5, 0, GPIO_CFG_OUTPUT,
			GPIO_CFG_PULL_DOWN, GPIO_CFG_2MA),
...
static void lcdc_trebon_gpio_init(void)
{
	...
	if (gpio_request(GPIO_SPI_SDO, "gpio_spi_sdo")) {
		pr_err("failed to request gpio spi_sdo\n");
	}

	for (n = 0; n < sizeof(lcdc_amazing_gpio_init_table)
				/ sizeof(uint32_t); n++) {
		rc = gpio_tlmm_config(lcdc_amazing_gpio_init_table[n],
					GPIO_CFG_ENABLE);
		if (rc) {
			printk(KERN_ERR "%s: lcdc_amazing_config(%#x)=%d\n",
			__func__, lcdc_amazing_gpio_init_table[n], rc);
			break;
			}
		}
...
static uint32_t lcdc_gpio_table[] = {
	...
	GPIO_CFG(GPIO_SPI_SDO, 0, GPIO_CFG_INPUT,
		GPIO_CFG_NO_PULL, GPIO_CFG_2MA),
};
...
#ifdef CONFIG_TOUCHSCREEN_MELFAS_TS
static struct tsp_callbacks * tsp_charger_callbacks;

static void  vital2_inform_charger_connection(int mode)
{
	if (tsp_charger_callbacks && tsp_charger_callbacks->inform_charger)
		tsp_charger_callbacks->inform_charger(tsp_charger_callbacks, mode);
};

static void register_tsp_callbacks(struct tsp_callbacks *cb)
{
	tsp_charger_callbacks = cb;
}
...
static struct i2c_board_info touch_i2c_devices[] = {
	...
	//      },
	{
		I2C_BOARD_INFO("melfas_ts_i2c", MELFAS_TSP_ADDR),
		.irq = MSM_GPIO_TO_INT( GPIO_TOUCH_IRQ ),
	},
};
#endif
...
static void __init ram_console_setup()
{
	struct membank *bank = &meminfo.bank[0];

	while (bank->start != CONFIG_ANDROID_RAM_CONSOLE_BANK_ADDR)
		bank++;

	ram_console_resources[0].start = bank->start + bank->size;
	ram_console_resources[0].end = ram_console_resources[0].start +
				CONFIG_ANDROID_RAM_CONSOLE_BUFFER_SIZE - 1;
}
#endif
...
#if 0  /*FSA Driver Porting: GPIO 49 and 50 used by MUS_SDA and MUS_SCL */
...
static void __init msm_device_i2c_init(void)
{
	...
	gpio_tlmm_config(GPIO_CFG(GPIO_MUSB_INT, 0, GPIO_CFG_INPUT, GPIO_CFG_PULL_UP, GPIO_CFG_2MA),GPIO_CFG_ENABLE);
...
static unsigned int kp_row_gpios[] = {39};
static unsigned int kp_col_gpios[] = {36, 31};
...
static void __init msm7x2x_init(void)
{
	...
	msm_device_gadget_peripheral.dev.platform_data =
		&msm_gadget_pdata;
	/*	msm7x27a_cfg_smsc911x(); */ /* FSA driver porting */
#ifdef CONFIG_SAMSUNG_JACK
	sec_jack_gpio_init();
#endif
...
/*	LED_onoff(1); func call moved to probe in gp2a.c*/
#endif
